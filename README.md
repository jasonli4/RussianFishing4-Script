<p align="left">
  ...
  <p align="left">
        一款功能完善的自动钓鱼脚本，配备集成式 UI，支持 手杆、水底、路亚 以及 海图全天候钓鱼模式，并内置连点器功能。  
  </p>
...
- **窗口**：仅支持**全屏无边框窗口**。
- **语言支持**：仅支持**简体中文界面**及**简体中文游戏界面**。
- **游戏内鼠标设置**：灵敏度仅支持**默认值**。
- **模式切换**：切换模式后需先停止程序，再重新启动。
- **后台运行**：如需后台运行，建议在高配置环境下使用虚拟机（如 VMware）。

### 手杆模式
- 配置好鱼竿并取出，启动程序，仅支持**单杆**。
- 设置中打开浮子相机，始终显示-中下-小
- 流水状态下视角会自动向右转动保持顺水漂流状态，静水视角保持不变
